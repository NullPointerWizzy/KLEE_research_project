...
int main (int argc, char *argv[]) {
  ...
  klee_make_symbolic (&unique, sizeof (unique), "unique");
  size_grid = klee_range (0, 9, "size_grid");
  bool mode_value = false;
  klee_make_symbolic(&mode_value,sizeof(mode_value),"mode_value");
  if(mode_value){
    mode = mode_all;
  }
